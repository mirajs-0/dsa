...
class IntArray():
    ...
    def insert(self, index:int, val:int) -> None:
        if not isinstance(index, int)or not 0 <= index <= self._size:
            raise IndexError(f"Index must be between 0 and {self._size} inclusive.")
        if not isinstance(val, int) or not self._min_val <= val <=self._max_val:
            raise TypeError(f"Value must be an integer between {self._min_val} and {self._max_val}")
        ...
        self._size += 1
        new = ReservedMemory(self._size*bpe)
        
        if index > 0:
            new.copy(old, count = index*bpe)
            
        if old and index < self._size -1:
            new.copy (old,
                        count=(self._size-1-index)*bpe,
                        source_index = index*bpe,
                        destination_index=(index+1)*bpe)
        self._resmem = new

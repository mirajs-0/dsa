...
class IntArray():
    ...
    def remove(self, index:int) -> int:
        ...
        if not isinstance(index, int) or not 0 <= index < self._size:
            raise IndexError(f"Index must be between 0 and {self._size-1}")
        ...
        if new_size > 0:
            new = ReservedMemory(new_size*bpe)
            if index > 0:
                new.copy(self._resmem, count=index*bpe)
            tail_count = old_size - index -1
            if tail_count > 0:
                new.copy(self._resmem, count=tail_count*bpe, source_index=(index+1)*bpe, destination_index=index*bpe)
                self._resmem = new
        else:
                self._resmem = None
